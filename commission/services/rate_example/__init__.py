# commission/services/rate_example/__init__.py
"""
RateExample 서비스 패키지.

이 패키지는 commission/services/rate_example.py 를 패키지로 전환한 것이다.
기존 import 경로 `from commission.services.rate_example import RateExampleService` 는
이 __init__.py 를 통해 그대로 작동한다.

하위 구조:
- common/  : 공통 helper (decimal, excel, pdf, rows, text)
- life/    : 생명보험 보험사별 정규화 모듈
- fire/    : 손해보험 보험사별 정규화 모듈 (예정)
"""
import logging
import os
...
class RateExampleService:
    ...
    def create(
        *,
        ...
        # ─────────────────────────────────────────────────────
        # KDB/교보/농협/동양/라이나/메트/미래/삼성/푸본현대/하나/흥국 생명보험 환산율/수정률
        ...
        if (
            insurer_type == RateExample.TYPE_LIFE
            and category == RateExample.CAT_CONV
            and insurer in {"KDB", "교보", "농협", "동양", "라이나", "메트", "미래", "삼성", "푸본현대", "하나", "흥국"}
        ):
            product_kind = ""
