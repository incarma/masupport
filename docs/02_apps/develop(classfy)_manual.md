...
> 기준 버전: 2026-04-30 Rev.2  
> 반영 범위: `guide_core_infra.md`, `develop_core_infra.md`, 기존 `develop(classfy)_core_infra.md`, CORE INFRA 즉시조치급 패치, CSP 2단계 패치, 그리고 2026-04-30 추가 재패치 결과  
> 현재 상태: **Critical 1차 보안 패치 완료 + CSP script 단계 완료 + commission fail token 보호 완료 + style CSP/HTML sanitize/cleanup 단계 진행 예정**
...
- `django_ma` 프로젝트의 보안/성능 패치 진행 순서 결정
...
├─ views/approval.py
├─ views/downloads.py
├─ views/utils_fail_excel.py
├─ views/_files.py
└─ views/_excel_export.py

dash/
├─ viewmods/api_upload.py
├─ viewmods/api_retention_upload.py
└─ templates/dash/*.html
...
| IP 신뢰 | `X-Forwarded-For` CIDR 기반 신뢰 검증 | 완료 | `audit/utils.py`, `web_ma/settings.py` |
| IP 신뢰 | `AUDIT_PROXY_HEADER_ENABLED` 토글 추가 | 완료 | `audit/utils.py`, `web_ma/settings.py` |
| 파일 권한 | 계정 업로드 결과 파일 owner 검증 | 완료 | `accounts/views.py`, `accounts/admin.py` |
| 파일 권한 | owner 없는 legacy task fallback을 superuser로 축소 | 완료 | `accounts/views.py` |
...
### 3.3 완료된 추가 재패치 Rev.2

| 구분 | 항목 | 현재 상태 | 기준 파일 |
|---|---|---:|---|
| Audit IP | `PHONE_RE`, `SSN_RE` 중복 선언 정리 | 완료 | `audit/utils.py` |
| Audit IP | proxy header 신뢰 토글 추가 | 완료 | `AUDIT_PROXY_HEADER_ENABLED` |
| Accounts | 빈 `task_id` progress 요청 처리 순서 보정 | 완료 | `accounts/views.py` |
| Accounts | owner 없는 legacy upload task fallback을 `superuser`만 허용 | 완료 | `accounts/views.py` |
| Commission | fail token payload에 `owner_id` 저장 | 완료 | `commission/views/utils_fail_excel.py` |
| Commission | deposit upload fail token owner 저장 | 완료 | `commission/views/api_upload.py` |
| Commission | approval/efficiency upload fail token owner 저장 | 완료 | `commission/views/approval.py` |
| Commission | fail excel 다운로드 로그인/권한/owner 검증 | 완료 | `commission/views/downloads.py` |
| Commission | approval pending / efficiency excess 다운로드 superuser 제한 | 완료 | `commission/views/downloads.py` |

---

## 4. 현재 남은 주요 보완 영역

아래 항목은 최신 기준에서 아직 남은 보완 후보이다.

| 구분 | 항목 | 등급 | 상태 |
|---|---|---:|---|
| CSP style | `style-src 'unsafe-inline'` 제거 | 최상~상 | 진행 예정 |
| CSP style | 템플릿 `style=""`, `<style>` 외부 CSS화 | 최상~상 | 진행 예정 |
| Manual XSS | `ManualBlock.content` / `{{ b.content|safe }}` sanitize 정책 점검 | 최상~상 | 진행 예정 |
| 파일 접근 | `.file.url`, `.image.url` 잔존 전수 grep | 최상~상 | 진행 예정 |
| 파일 lifecycle | `UPLOAD_TEMP_DIR`, `UPLOAD_RESULT_DIR`, fail token cleanup | 중상~상 | 진행 예정 |
| RequestLog | RequestLog/AuditLog retention/cleanup | 중상 | 진행 예정 |
| Upload 검증 | Dash/Commission/Partner 업로드 크기/확장자/MIME 검증 전수 보강 | 상 | 진행 예정 |
| Partner | efficiency confirm / ratetable upload MIME/size 검증 보강 | 상 | 진행 예정 |
| Docker | image tag, collectstatic, 운영 bind mount 점검 | 상~중상 | 진행 예정 |
...
- 업로드 결과 token 또는 fail token이 사용자/권한과 바인딩되지 않아 타인이 다운로드 가능

#### 최신 상태

| 항목 | 현재 상태 | 기준 |
|---|---:|---|
| `/media/` 직접 접근 | 완료 | Nginx 403 + 보호 view 원칙 |
| `.file.url` 직접 링크 주요 앱 제거 | 대부분 완료 | board/manual/partner 주요 경로 보호 view |
| 계정 업로드 결과 권한 | 완료 | owner cache + path 검증 |
| commission fail token owner 검증 | 완료 | owner_id 저장 + superuser/owner 검증 |
| commission 정산 다운로드 권한 | 완료 | superuser 제한 |
| `X-Forwarded-For` 무조건 신뢰 | 완료 | proxy toggle + trusted CIDR |
| `csrf_failure` cookie 원문 로그 | 완료 | cookie `***` 처리 |
| CSP `unsafe-eval` | 완료 | 제거됨 |
| 주요 inline script | 완료 | bridge/external JS 전환 |
| `style-src 'unsafe-inline'` | 미완료 | 다음 단계 |
| manual HTML content sanitize | 미완료 | 다음 단계 |
...
| Commission upload CSRF | 일부 `csrf_exempt` 존재 | superuser 전용이지만 CSRF 정책 점검 필요 |
...
| Upload temp/result cleanup | 미완료 | Celery cleanup task 후보 |
| RequestLog retention | 미완료 | cleanup task 후보 |
| AuditLog retention | 미완료 | 정책 결정 필요 |
| Celery idempotency | 일부 기준화 | board/dash/commission 전수 점검 필요 |
| cache result owner | accounts/commission fail token 완료 | 기타 token 전수 점검 필요 |
...
| `X-Forwarded-For` 신뢰 검증 | 완료 | `AUDIT_PROXY_HEADER_ENABLED` + `AUDIT_TRUSTED_PROXY_CIDRS` |
| accounts 업로드 결과 파일 권한 검증 | 완료 | owner cache + fallback superuser 제한 |
| accounts 결과 파일 path traversal 차단 | 완료 | `UPLOAD_RESULT_DIR` 내부만 허용 |
| commission fail token owner 검증 | 완료 | token payload owner_id + download 검증 |
| commission 정산 엑셀 다운로드 권한 | 완료 | superuser 제한 |
...
| `.file.url`/`.image.url` 잔존 | 객체 권한 없는 파일 접근 가능성 | 전체 template grep |
...
| `csrf_exempt` 잔존 | 일부 commission upload | 제거 가능성 또는 CSRF 대체 방어 점검 |

---

## 8. 최신 Moderate-High 상태표
...
| cache TTL/owner 바인딩 | accounts/commission 완료 | 기타 token 전수 점검 |
...
### 11.2 2순위: Manual content sanitize
...
- `manual_detail.html`에서 `{{ b.content|safe }}` 렌더링 구조가 존재함
- Quill 기반 HTML 저장 구조에서는 허용 태그/속성 sanitizer 정책이 필요함
- CSP style 정리와 함께 XSS 방어 최종 단계로 묶어 처리하는 것이 적절함
...
- 서버 저장 전 sanitize 또는 렌더 전 sanitize 정책 확정
- 허용 태그/속성 allowlist 명시
- `script`, event handler, javascript: URL, 위험 style 제거
- 기존 매뉴얼 콘텐츠 회귀 최소화

### 11.3 3순위: upload cleanup/retention
...
### 11.4 4순위: upload validation 전수 보강
...
### 11.5 5순위: Docker/Nginx 운영 안전성 추가 점검
...
- 운영 bind mount 정책 확인
- staticfiles/manifest collectstatic 절차 확인
- image tag 고정
- healthcheck와 maintenance fallback 확인
- `/media/` 직접 서빙 금지 유지
...
### 13.7 CSP inline style 제거 대상

```bash
rg "style=|<style>" templates
```
...
accounts upload result owner 검증: 완료
commission fail token owner 검증: 완료
commission 정산 다운로드 권한 제한: 완료
proxy IP 신뢰 검증: 완료
CSP style 정리: 진행 예정
manual content sanitize: 진행 예정
...
2. manual content sanitize 정책 확정 및 적용
