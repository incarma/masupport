...
# 손해보험 지급률(FIRE PAY) 추가 보정 정책 — Final SSOT (2026-05-18 추가)

## 개요

손해보험 지급률 정규화 로직(`fire_pay.py`)의 운영 중 발견된 다음 이슈를 수정한다.

1. 메리츠 지급률 블록이 정규화되지 않는 문제
2. 보험사명이 `구 분`으로 잘못 저장되는 문제
3. `라이나손보` canonical 불일치 문제
4. 손해보험 지급률 `/0.97` 보정 저장 정책 명문화
...
기존 parser는 다음 헤더가 정확히 모두 존재할 때만 보험사 블록으로 인정했다.

```text
초회 | 2~6회 | 7~12회 | 13회 | 14회 | 15회
```

그러나 메리츠 RAW는:

```text
초회 | 2~6회 | 7~12회 | 13회 | 14회 | 계
```

구조를 사용한다.

따라서 메리츠 전체 block이 skip되었다.

---

# 최종 정책

고정 6칸 완전 일치 방식 사용 금지.

다음 필수 헤더만 존재하면 보험사 블록으로 인정한다.
...
손해보험 지급률은 생명보험 지급률과 동일하게 저장 전 `/0.97` 보정을 적용한다.
...
```powershell
python manage.py shell -c "from commission.models import RateExamplePayRow as R; print(R.objects.filter(insurer='메리츠').count())"
```
...
```powershell
python manage.py shell -c "from commission.models import RateExamplePayRow as R; print(R.objects.filter(insurer__contains='구').count())"
```
...
```powershell
python manage.py shell -c "from commission.models import RateExamplePayRow as R; print(R.objects.filter(insurer='라이나').count())"
```
...
from commission.models import RateExamplePayRow as R
r = R.objects.filter(insurer='현대').first()
print(r.col_first)
"
```
...
[ ] 15회 없는 보험사 정상 처리
[ ] 보험사=구 분 row 0건
[ ] 라이나손보 → 라이나 canonical 정상
[ ] 지급률 /0.97 보정 정상
[ ] 생명보험 지급률 영향 없음
[ ] 손해보험 수정률 영향 없음
[ ] 기존 DB/KB/농협/삼성/롯데/한화/현대/AIG 기능 영향 없음
```
...
```text
보험사 block은 동적 헤더 매핑 기반으로 처리
메리츠처럼 15회 없는 구조 허용
보험사명 "구 분" 저장 금지
라이나손보 canonical → 라이나
손해보험 지급률은 저장 전 /0.97 보정
```


---

# 메리츠화재(FIRE) 수정률 정규화 — Final SSOT (2026-05-18 추가)

## 개요

메리츠화재 수정률 RAW/PDF 파일을 `RateExampleConversionRow` 기준으로 정규화한다.
...
```python
commission/services/rate_example_normalizers/fire_meritz.py
```

핵심 목적:

- 메리츠 손해보험 수정률 RAW를 손해보험 수정률 단일 컬럼 구조(`year1`)로 정규화한다.
- 담보명에 `기본계약` 또는 `반려견`이 포함된 row만 저장한다.
- 보험사 canonical은 `메리츠`로 고정한다.
- `구분` 컬럼이 비어 있으면 `사용안함`으로 저장한다.
- 납기는 `납입기간 + 보험기간` 조합으로 저장한다.
- 수정률은 raw 백분율 값을 그대로 저장한다.
...
| 보험사 | insurer | `메리츠` 고정 |
| 상품군 | coverage_type | 보장 / 보장(태아) / 연금 / 저축 / 단독실손(초회) / 단독실손(갱신) |
| 상품명 | product_name | 담보명 기준 |
| 구분 | plan_type | 공란 시 `사용안함` |
| 납기 | pay_period | 납입기간 + 보험기간 |
| 수정률 | year1 | raw 백분율 그대로 저장 |
| 미사용 | year2~year4 | `None` 저장 |
...
```python
if "기본계약" in title or "반려견" in title:
```
...
from commission.services.rate_example_normalizers.fire_meritz import (
    build_fire_meritz_conversion_rows,
)
```
...
```python
    ...
    normalized_rows.extend(
        build_fire_meritz_conversion_rows(example, wb)
    )
```
...
```powershell
python manage.py shell -c "from commission.models import RateExampleConversionRow as R; print(R.objects.filter(insurer_type='fire', category='conv', insurer='메리츠').delete())"
```

재업로드:

```text
손해보험 / 수정률 / 메리츠 / replace
```

검증:

```powershell
python manage.py shell -c "from commission.models import RateExampleConversionRow as R; qs=R.objects.filter(insurer_type='fire', category='conv', insurer='메리츠'); print(qs.count()); print(list(qs.values('coverage_type','product_name','plan_type','pay_period','year1')[:20]))"
```
...
```text
[ ] 메리츠 업로드 시 normalized_count > 0
[ ] 수정률 조회 모달에서 보험사=메리츠 조회 가능
[ ] 기본계약 row만 저장되는지 확인
[ ] 반려견 상품 저장 정상
[ ] 구분 공란 → 사용안함 저장 정상
[ ] 납기 조합 정상
[ ] 수정률 130 → 130% 표시
[ ] DB/KB/농협/롯데/삼성/한화/현대/AIG 영향 없음
[ ] 생명보험 환산율/지급률 영향 없음
```
...
- superuser 업로드 정책 유지
- replace 모드 재업로드 권장
- 기존 메리츠 row가 0건 생성된 이력이 있으면 삭제 후 재업로드 권장
