# django_ma/board/services/industry_recommend.py
# =========================================================
# Board Industry Recommend Service
# - 업계정보 추천/인기기사 로직 SSOT
# - 3단계부터 board가 실제 서비스 소유
# - DB 모델은 현재 support.models 를 그대로 재사용
# =========================================================
...
from board.constants_industry import INDUSTRY_RECOMMEND_MODEL_VERSION
...
def _base_queryset(topic: str = ""):
    ...
    - 활성 기사만 조회
    - 운영 숨김 기사 제외
    - 평균 평점 / 북마크 수 annotate
    """
    qs = (
        IndustryArticle.objects.filter(is_active=True, is_hidden=False)
        .annotate(
...
def get_major_articles(limit: int = 6, topic: str = "") -> List[IndustryArticle]:
    """
    ...
    for article in articles:
        ...
        article.recommend_model_version = INDUSTRY_RECOMMEND_MODEL_VERSION
...
def get_recommended_articles_for_user(user, limit: int = 6, topic: str = "") -> List[IndustryArticle]:
    """
    사용자 맞춤 추천 MVP
    ...
    prefs = IndustryUserPreference.objects.filter(user=user).select_related("article")
